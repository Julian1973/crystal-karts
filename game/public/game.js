...
import {loadProgress,saveProgress,SHOP,buy as buyItem,equip as equipItem,awardRace,dailyFor,completeDaily,awardShard,cupMedal} from './progression.js?v=78';
...
import {CRYSTALS,PICKUP_COUNT,collectCrystal,activateSkill,skillEffects} from './skills.js?v=78';
import {createWeather,stepWeather,traction} from './weather.js?v=75';
import {launchCrystal,advanceShots} from './combat.js?v=78';
...
import {partyViewports,controllerURL,qrSVG} from './party.js?v=78';
import {dailyCourse,dailyComplete} from './daily.js?v=78';
import {decodeTrack,encodeTrack} from './custom-track.js?v=78';
...
let splitScreen=false;const partyCameras=[];
// Crystal Garden carries on to the next course through its auto-start link only.
const launch=new URLSearchParams(globalThis.location?.search||'');
let gardenMode=(launch.get('garden')==='1'||(launch.get('daily')==='1'&&dailyCourse().garden))&&launch.get('race')==='1',garden=null;const gardenMeshes=[];
...
function recordGhost(force=false){if(customTrack||room||practice||!runStats?.eligible||elapsed>600)return;if(!force&&elapsed-lastGhostSample<.1)return;
...
function saveRunRewards(rank){if(practice||!runStats)return;const ids=[];
 if(player.time!==null){ids.push('racer:'+characters[selected].id);if(!customTrack)ids.push('track:'+trackId);
 ...
 const best=loadGhost();if(!room&&!customTrack&&runStats.eligible){recordGhost(true);if(!best||player.time<best.time){
...
const characters=[{id:'keen',name:'Keen',trait:'Courage',color:0x19b9dd,fur:0x16aed1,note:'Courage in every corner'},{id:'aida',name:'Aida',trait:'Confidence',color:0xbe7ee9,fur:0xc4a1d4,note:'Believe in your next move'},{id:'sunny',name:'Sunny',trait:'Optimism',color:0xffc347,fur:0xf8b631,note:'A little sunshine at full speed'},{id:'misty',name:'Misty',trait:'Trust',color:0xb8d9ef,fur:0xe3e8fa,note:'Trust yourself through the turns'},{id:'amie',name:'Amie',trait:'Joy',color:0xf27da8,fur:0xe86a9a,note:'Every adventure is better together'},{id:'howey',name:'Howey',trait:'Kindness',color:0x83a2a8,fur:0x68757a,note:'A big heart on the starting line'},{id:'luna',name:'Luna',trait:'Calm',color:0xb698dc,fur:0xb89bd1,note:'A calm mind in every corner'}];
...
const requestedTrack=new URLSearchParams(globalThis.location?.search||'').get('track'),customTrack=decodeTrack(new URLSearchParams(globalThis.location?.search||'').get('custom')||''),trackId=customTrack?customTrack.theme:Object.hasOwn(TRACKS,requestedTrack)?requestedTrack:'wood';
// Track Builder courses keep the theme's scenery, sky and music but use the kid's own road.
if(customTrack)TRACKS[trackId]={...TRACKS[trackId],name:customTrack.name,description:'Made in the Track Builder',owner:'all',points:customTrack.points};
...
const gardenGoldMat=mat(0xffd66e,{metalness:.35,roughness:.18,emissive:0xf9b844,emissiveIntensity:.9});
function updateGardenVisuals(){const gems=garden&&mode==='racing'?garden.gems:[];while(gardenMeshes.length<gems.length)gardenMeshes.push(mesh(gemGeo,crystalMaterials.aida,0,0,0,.7,1.1,.7));
 for(let i=0;i<gardenMeshes.length;i++){const m=gardenMeshes[i],gem=gems[i];m.visible=!!gem&&gem.active;if(!m.visible)continue;const f=frame(gem.s,gem.lane);m.position.copy(f.p);m.position.y+=1.5+Math.sin(elapsed*3+i)*.25;m.rotation.y=elapsed*2+i;m.material=gem.golden?gardenGoldMat:crystalMaterials[characters[i%characters.length].id]||crystalMaterials.aida;const scale=gem.golden?1.4:1;m.scale.set(.7*scale,1.1*scale,.7*scale);}}
function finishGarden(){const ranking=gardenRanking(garden,[player,...bots],length);for(const r of ranking)if(r.time===null)r.time=elapsed;finished=ranking;const final=garden;finish();const rank=ranking.indexOf(player)+1,suffix=['st','nd','rd'][rank-1]||'th';$('finish-title').textContent=rank===1?'Crystal Garden champion!':'What a crystal garden!';$('finish-summary').textContent=characters[selected].name+' gathered '+gardenScore(final,player)+' crystals · '+rank+suffix+' place';$('leaderboard').querySelectorAll('.result-row span').forEach((span,i)=>{if(ranking[i])span.textContent=gardenScore(final,ranking[i])+' ◆';});$('results-label').textContent=track.name+' · CRYSTAL GARDEN';garden=null;}
...
function beginCup(id='whisper'){if(!validCupId(id))return false;timeTrial=false;ghostMode=false;practice=false;gardenMode=false;if(room){toast('Leave your online room to start a solo Cup.');return false}try{cup=newCup(selected,id);}catch{return false}if(saveCup()){location.href='?track='+tourTracks(cup)[0]+'&cup=1';return true}return false;}
function updateRace(dt){
 ...
 updateResonanceBeam();updateGardenVisuals();
...
function physicsStep(dt){
 ...
 for(const b of bots){
  ...
  const targetObject=(garden&&nearestGem(garden,b,tuning.lookahead,wrapDelta))||(box&&(!pad||wrapDelta(box.s-b.s,length)<wrapDelta(pad.s-b.s,length))?box:pad);if(targetObject){const d=wrapDelta(targetObject.s-b.s,length),weight=THREE.MathUtils.clamp(1-d/tuning.lookahead,.08,.68);b.aiRacingLane+=(targetObject.lane-b.aiRacingLane)*weight;}
 ...
 if(garden)for(const e of stepGarden(garden,racers,elapsed,wrapDelta)){puff(racerFrame(e.r).p,e.gem.golden?0xffd66e:characters[e.r.ci].color);if(e.r===player){audio.effect('crystal');if(e.gem.golden){toast('Golden crystal! +'+e.value);triggerBigMoment(0xffd66e);}}}
 ...
 for(const r of racers)updateProgress(r,previousPositions.get(r),length);rareShard?.update(player,elapsed);if(!customTrack&&!practice&&!ghostMode&&!timeTrial&&!room&&!roundB.shards.includes(trackId)){const shardAt=secretShardAt,was=previousPositions.get(player)??player.s;if(player.completedLaps>=1&&was<length+shardAt&&player.s>=length+shardAt&&Math.abs(player.lane)<1.35&&player.speed>12){awardShard(roundB,trackId);saveRoundB();const owned=readShards();owned[trackId]=true;try{localStorage.setItem('crystal-course-shards-v1',JSON.stringify(owned))}catch{}shardCollection=owned;secretShard.visible=secretShardRing.visible=false;toast(track.name+' secret shard found! '+roundBShardCount()+' / 11');audio.effect('crystal');if(roundBShardCount()===11){roundB.owned??=[];if(!roundB.owned.includes('golden-kart'))roundB.owned.push('golden-kart');saveRoundB();}}}
 ...
 if(!garden&&player.completedLaps>priorLaps&&player.completedLaps<3){toast(player.completedLaps===2?'Final lap! You’ve got this.':'Lap 2 · Keep your courage!');tone(700,.3);if(player.completedLaps===2&&audio.music)audio.music.playbackRate=1.07}
 ...
 if(garden){if(elapsed>=GARDEN.duration)finishGarden();return}
...
function updateHUD(){if(performance.now()-lastHUD<100)return;lastHUD=performance.now();
 const pos=garden?1+gardenRanking(garden,[player,...bots],length).indexOf(player):1+bots.filter(b=>b.s>player.s).length,power=CRYSTALS[player.powerTime>0?player.activePower:player.heldPower||player.id],item=ITEM_POWERS[player.itemQueue?.[0]||player.itemPower],effects=skillEffects(player),positionEl=$('position');
 if(positionEl.dataset.rank!==String(pos)){if(positionEl.dataset.rank){positionEl.classList.remove('position-pop');void positionEl.offsetWidth;positionEl.classList.add('position-pop')}positionEl.dataset.rank=String(pos);tone(520+Math.max(0,10-pos)*35,.09,'sine',.035,700)}positionEl.innerHTML=pos+'<span>'+(['st','nd','rd'][pos-1]||'th')+'</span>';$('racer-count').textContent=garden?'CRYSTAL GARDEN · '+gardenScore(garden,player)+' ◆':timeTrial?'TIME TRIAL':ghostMode?'YOUR BEST GHOST':practice?'PRACTICE':'OF '+(bots.length+1)+' RACERS';
 ...
 $('lap').previousElementSibling&&($('lap').previousElementSibling.textContent=garden?'TIME LEFT':'LAP');$('lap').textContent=garden?'⏱ '+fmt(gardenTimeLeft(garden,elapsed)):practice?'Practice':displayedLap(player)+' / 3';$('time').textContent=timeTrial?displayTime(Math.round(elapsed*1000)):fmt(elapsed);$('speed').textContent=Math.round(Math.abs(player.driveSpeed)*3.6);document.body.classList.toggle('speed-rush',Math.abs(player.driveSpeed)*3.6>40&&!mobileDevice);
...
const raceMap=createRaceMap($('map'),track);function drawMap(){raceMap.draw(player,bots,characters);$('map-title').textContent=garden?'YOU · '+gardenScore(garden,player)+' ◆':practice?'YOU · PRACTICE':'YOU · LAP '+displayedLap(player)+'/3';}
...
// Party mode: phones scan this to join as controllers; the host screen runs and shows the race.
function showPhoneJoin(){let panel=document.getElementById('phone-join');if(!room?.host){panel?.remove?.();return}if(!panel){panel=document.createElement('div');panel.id='phone-join';$('room-lobby').insertBefore(panel,$('room-members'));}
 const link=controllerURL(location.href,room.code,trackId);if(panel.dataset.link===link)return;panel.dataset.link=link;
 panel.innerHTML='<div class="phone-qr">'+qrSVG(qrcode,link)+'</div><div class="phone-copy"><strong>Party mode · phones as controllers</strong><p>Scan with a phone camera, pick a bear, then race together on this screen.</p><small>'+link+'</small><label><input type="checkbox" id="split-screen"'+(splitScreen?' checked':'')+'> Split this screen for phone racers (up to 4 views)</label></div>';
 const box=document.getElementById('split-screen');if(box)box.onchange=e=>{splitScreen=!!e.target.checked};}
// Split-screen: the host's view plus up to three phone racers, each with a chase camera.
function renderParty(dt){if(!splitScreen||!room?.host||!onlineStarted||(mode!=='racing'&&mode!=='countdown'))return false;const views=[player,...bots.filter(b=>b.manual)].slice(0,4);if(views.length<2)return false;
 const rects=partyViewports(views.length,innerWidth,innerHeight);renderer.setScissorTest(true);
 views.forEach((r,i)=>{const [x,y,w,h]=rects[i];let cam=camera;if(i>0){cam=partyCameras[i-1]??=new THREE.PerspectiveCamera(60,1,.1,1800);const f=racerFrame(r,r.s,r.lane),fwd=cameraForward.set(Math.sin(r.heading||0),0,Math.cos(r.heading||0)),target=cameraTarget.copy(f.p).addScaledVector(fwd,-11);target.y+=6.5;cam.position.lerp(target,1-Math.exp(-dt*9));const look=cameraAhead.copy(f.p).addScaledVector(fwd,16);look.y+=1.7;cam.lookAt(look);}
  cam.aspect=w/h;cam.updateProjectionMatrix();renderer.setViewport(x,y,w,h);renderer.setScissor(x,y,w,h);renderer.render(scene,cam);});
 renderer.setScissorTest(false);camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();return true;}
async function openRoom(action){
 if(customTrack){$('room-message').textContent='Custom tracks are for solo races and Crystal Garden for now. Pick a real course to race online.';return}
...
$('practice-start').onclick=()=>{if(!room&&!cup){timeTrial=false;ghostMode=false;practice=true;gardenMode=false;start()}else toast('Return to the garage first to practise.')};
{const today=dailyCourse(),done=roundB.daily.date===today.date&&roundB.daily.done;$('daily-start').textContent='☀ Today’s Crystal Cove · '+today.title+(done?' ✓':'');$('daily-start').title=today.text;}
$('daily-start').onclick=()=>{const d=dailyCourse();if(room||cup){toast('Return to the garage first to play today’s course.');return}try{sessionStorage.setItem('kart-racer',String(d.bear))}catch{}if(d.track!==trackId){location.href='?track='+d.track+'&step=track&race=1&daily=1';return}selected=d.bear;renderCharacters();makeRacers();timeTrial=false;ghostMode=false;practice=false;gardenMode=d.garden;toast(d.title+' · '+d.text);start();};
$('garden-start').onclick=()=>{if(!room&&!cup){timeTrial=false;ghostMode=false;practice=false;gardenMode=true;start()}else toast('Return to the garage first to play Crystal Garden.')};
refreshPaints();
...
function setupMenuFlow(){
 ...
 for(const id of ['daily-start','builder-open','garden-start','practice-start','ghost-start','sticker-open','online','cup-start','phone-setup'])$('flow-extras').append($(id));
 ...
 for(const button of document.querySelectorAll('[data-course]'))button.onclick=()=>{if(button.dataset.course===trackId){ghostMode=false;practice=false;gardenMode=false;raceRequested=true;if(modelsReady)start();else $('model-status').textContent='Getting your racer ready…';return;}$('track-select').value=button.dataset.course;$('track-select').dispatchEvent(new Event('change'))};
...
trialUI=createTrialUI({tracks:TRACKS,characters,choose:id=>{
 ...
 timeTrial=true;practice=false;gardenMode=false;trialTarget=null;trialUI?.clearTarget();for(const b of document.querySelectorAll('[data-course]'))b.hidden=false;
...
$('next-track').onclick=()=>{const ids=Object.keys(TRACKS);location.href='?track='+ids[(ids.indexOf(trackId)+1)%ids.length]+'&step=track&race=1'+(timeTrial?'&tt=1':'')+(gardenMode?'&garden=1':'');};
$('ghost-start').onclick=()=>{timeTrial=true;if(room||cup){toast('Return to the garage to race your best.');return;}ghostMode=true;practice=false;gardenMode=false;start();};
